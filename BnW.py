...
def playSound(interval=0, sec=4, winsound=None):
    while interval >= 0:
...
def g2nn1(description):
    ...
    for i in range(0, len_description):  # taking eculidean distance of all points
        ...
        for j in range(0, len_description):
            if i != j:
                dist = distance.euclidean(description_object[i].description,
                                          description_object[j].description)  # computing euclidean distance
                distance_keypoint.append(
                    desc_obj(dist, description_object[i].key_coordinates, description_object[j].key_coordinates))
    ...
    for i in range(1, len(vector) - 1):
        if vector[i].size > MIN_CLUSTER_SIZE_CONTROL:
            for j in range(i + 1, len(vector)):
                if vector[i].size > MIN_CLUSTER_SIZE_CONTROL:
                    matched, count, full_cluster1, full_cluster2 = checkClusters(table, table1, table2, vector[i],
                                                                                 vector[j], matched_kp)
...
def drawLinesOnImage(f1, f2, cluster_img):
    ...
    for i in range(0, len(f1), 2):
        cv2.line(img, (int(f1[i].pt[0]), int(f1[i].pt[1])), (int(f2[i].pt[0]), int(f2[i].pt[1])), (0, 255, 0), 1,
                 cv2.LINE_AA)
...
def main(file_name):
    ...
    #print(json.dumps(g2nn_output.__dict__))
...
if __name__ == "__main__":
    ...
    #print("=============>", DIR)
    '''for f in os.listdir("/home/prashantmaurya/Desktop/Copy-Move-Image-Forgery-Detection/Input/"):
        print(f)
    list_dir = os.listdir(DIR)'''
    DIR = "/home/prashantmaurya/Desktop/Copy-Move-Image-Forgery-Detection/Input/"
    list_dir = os.listdir(DIR)
